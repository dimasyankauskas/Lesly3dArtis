...
- **`v1-brand.css` is the Direction 1 design system** (dark editorial, gold accents, Playfair Display + Montserrat). It is loaded *in addition to* `styles.css` on both pages. All its selectors are scoped to `:root[data-version="v1"]` so it has zero effect when V2 is active. Used to A/B compare the two design directions extracted from the 9 mockups in `reserch/visuals/`.
...
- **Version system (V1 / V2 design):** the `<html>` element carries `data-version="v2"` (default, Quiet Premium cream) or `data-version="v1"` (dark editorial + gold). The inline FOUC-prevention script reads `localStorage['lesly-version']`. The toggle in the header (`<div class="version-toggle">` with two `<button data-version-set="...">` children) flips the attribute and persists. All `v1-brand.css` selectors are scoped to `:root[data-version="v1"]`, so V2 is the unstyled default. Used to compare the two extracted directions live before locking the choice.
...
- **Swap a placeholder for real media** — first verify the image is actual portfolio content (a character render, turnaround, wireframe, texture, or portrait) by reading it with the Read tool and looking at what it depicts. If the image is a page mockup, layout reference, or AI-generated stock, do NOT use it. Once verified, replace the placeholder `<div class="thumb">` (or `.case-hero`, `.case-hero-bg`, `.about-portrait`) block with a `<picture>` element containing a `<source type="image/webp">` and a fallback `<img src="reserch/visuals/imageN.png" alt="..." width height loading="lazy">`. Use WebP at q=80 (~70% size reduction) and JPG q=85 fallback. Always include explicit `width` and `height` to prevent layout shift. Use `loading="eager" fetchpriority="high"` for above-the-fold hero images; `loading="lazy"` everywhere else.
...
- **The `reserch/visuals/image*.png` files are FULL WEBSITE PAGE MOCKUPS from a Direction 1 design exploration, NOT Lesly's character renders.** They contain embedded AI-generated stock character art, navigation, and template section names ("GAMER PORT", "MY LATEST PROJECTS", "WORK PROCESS", "STRATEGY MEETS CREATIVITY"). They must NEVER be used as portfolio content on the live site. Treat them strictly as design references. Until Lesly provides her actual character renders (ZBrush/Marmoset output, turnarounds, wireframes, texture closeups, plus a real portrait of Lesly), the site stays text-only.
- **VERIFY IMAGE CONTENT BEFORE INTEGRATING.** When given image files, read every one with the Read tool before swapping any of them into HTML placeholders. If the file is a page mockup, layout reference, AI-generated stock art, or anything other than actual portfolio content, stop and surface that to the user — do not assume, do not proceed with a guess, do not optimize images that may not be usable. The cost of a wrong swap is high (misrepresentation, copyright risk, design mismatch, time wasted). When in doubt, ask.
- **Verify `reserch/` docs say.** Source-of-truth for copy is `reserch/upgrade-jd-aligned-website-copy.md` (title, services, form fields), `reserch/website-information-architecture.md` (hero/structure), and `reserch/final-ai-workflow-update.md` (AI framing). Re-read these before any major copy change.
...
- The hero, portfolio, and about-portrait blocks currently render text-only placeholders — visuals are queued per `reserch/visual-assets-checklist.md` and `reserch/implementation-plan.md`.
